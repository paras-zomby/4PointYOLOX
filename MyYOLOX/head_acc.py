...
from .boxes_acc import bboxes_iou, bboxes_iou_batch
...
class YOLOXHead(nn.Module):
    ...
    def forward(self, xin, labels=None, nlabel=None):
        ...
        for k, (cls_conv, reg_conv, stride_this_level, x) in enumerate(
            ...
            if self.training:
                ...
                origin_reg_pred.append(reg_output.clone())
    ...
    def get_output_and_grid(self, output, k, stride, dtype):
        ...
        if grid.shape[2:4] != output.shape[2:4]:
            ...
            grid = torch.stack((xv, yv) * 4, 2).view(1, self.n_anchors, hsize, wsize, 8).type(dtype)
            self.grids[k] = grid

        output = output.view(batch_size, self.n_anchors, 9 + self.num_classes, hsize, wsize)
    ...
    def get_losses(
        ...
        bbox_preds = outputs[:, :, :8]              # [N, A, 4]   这是解码后的xywh。
        obj_preds = outputs[:, :, 8].unsqueeze(-1)  # [N, A, 1]   置信度。没有经过sigmoid()激活。
        cls_preds = outputs[:, :, 9:]               # [N, A, n_cls]   n_cls个类别的条件概率。没有经过sigmoid()激活。
        ...
        # 第三步，准备 回归分支 需要的监督信息。
        reg_targets = self.gather(gt_bboxes.reshape((N*G, 8)), matched_gt_inds)  # [num_fg, 4]  num_fg个最终正样本需要学习的预测框xywh。
        ...
        l1_targets = self.get_l1_target(
                torch.zeros((num_fg_int, 8), dtype=torch.float32, device=obj_preds.device),
                reg_targets,
        ...
        origin_reg_pred = torch.reshape(origin_reg_pred, [-1, 8])       # [N*A, 8]
        pos_origin_preds = self.gather(origin_reg_pred, pos_index)  # [num_fg, 8]
        loss_reg = (self.reg_loss(pos_origin_preds, l1_targets)).sum() / num_fg

        loss = self.obj_weight * loss_obj + self.cls_weight * loss_cls + self.reg_weight * loss_reg
        if self.use_iou_loss:
            loss += loss_iou
        return (
                loss,
                (loss_iou * self.iou_weight).item(),
                (loss_obj * self.obj_weight).item(),
                (loss_cls * self.cls_weight).item(),
                (loss_reg * self.reg_weight).item(),
                (num_fg / max(nlabel.sum(), 1)).item(),
                )
    ...
    def get_assignments(
        ...
        pair_wise_ious = IOULoss.get_IOU(
                bbox_preds[:, None, ...].tile(gt_bboxes.shape[1], 1, 1),
                gt_bboxes[..., None, :].tile(bbox_preds.shape[1], 1)
                )  # [N, G, A]  两两之间的iou。
        # 假gt 和 任意预测框 的iou置为0
    ...
    def get_in_boxes_info(
        ...
        # gt_bboxes.shape=[N, G, 8]  格式是cxcywh
        N = gt_bboxes.shape[0]
        ...
        gt_bboxes_points = gt_bboxes.unsqueeze(-2).repeat(
                1, 1, A, 1
                )
        ...
        is_in_boxes = IOULoss.if_inside_bounding_box(
                torch.stack((x_centers, y_centers), dim=-1),
                gt_bboxes_points
                ).float()
        ...
        # *改成了四个点坐标平均来确定中心点坐标
        # gt_bboxes [N, G, 8]
        gt_bboxes_x_centers = gt_bboxes[..., 0::2].sum(dim=-1) / 4
        gt_bboxes_y_centers = gt_bboxes[..., 1::2].sum(dim=-1) / 4

        gt_bboxes_l = torch.tile(gt_bboxes_x_centers.unsqueeze(-1), [1, 1, A]) \
                      - center_radius * expanded_strides_per_image.unsqueeze(0).unsqueeze(1)   # [N, G, A]   cx - r*s
        gt_bboxes_r = torch.tile(gt_bboxes_x_centers.unsqueeze(-1), [1, 1, A]) \
                      + center_radius * expanded_strides_per_image.unsqueeze(0).unsqueeze(1)   # [N, G, A]   cx + r*s
        gt_bboxes_t = torch.tile(gt_bboxes_y_centers.unsqueeze(-1), [1, 1, A]) \
                      - center_radius * expanded_strides_per_image.unsqueeze(0).unsqueeze(1)   # [N, G, A]   cy - r*s
        gt_bboxes_b = torch.tile(gt_bboxes_y_centers.unsqueeze(-1), [1, 1, A]) \
                      + center_radius * expanded_strides_per_image.unsqueeze(0).unsqueeze(1)   # [N, G, A]   cy + r*s
